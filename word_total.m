function varargout = word_total(varargin)
% WORD_TOTAL MATLAB code for word_total.fig
%      WORD_TOTAL, by itself, creates a new WORD_TOTAL or raises the existing
%      singleton*.
%
%      H = WORD_TOTAL returns the handle to a new WORD_TOTAL or the handle to
%      the existing singleton*.
%
%      WORD_TOTAL('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in WORD_TOTAL.M with the given input arguments.
%
%      WORD_TOTAL('Property','Value',...) creates a new WORD_TOTAL or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before word_total_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to word_total_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help word_total

% Last Modified by GUIDE v2.5 09-Dec-2013 16:13:58

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @word_total_OpeningFcn, ...
                   'gui_OutputFcn',  @word_total_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before word_total is made visible.
function word_total_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to word_total (see VARARGIN)

% Choose default command line output for word_total
handles.output = hObject;

set(hObject, 'Name', '전체 단어장');
...
set(t_table_total, 'ColumnWidth', {200 200 95 95 95 100});
...
% UIWAIT makes word_total wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = word_total_OutputFcn(hObject, eventdata, handles) 
...
if strcmp(e.NewData, '예')
    word_list_tabletotal{word_ind, 11}=1;
else
    word_list_tabletotal{word_ind, 11}=0;
end
