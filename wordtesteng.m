function varargout = wordtesteng(varargin)
% WORDTESTENG MATLAB code for wordtesteng.fig
%      WORDTESTENG, by itself, creates a new WORDTESTENG or raises the existing
%      singleton*.
%
%      H = WORDTESTENG returns the handle to a new WORDTESTENG or the handle to
%      the existing singleton*.
%
%      WORDTESTENG('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in WORDTESTENG.M with the given input arguments.
%
%      WORDTESTENG('Property','Value',...) creates a new WORDTESTENG or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before wordtesteng_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to wordtesteng_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help wordtesteng

% Last Modified by GUIDE v2.5 11-Dec-2013 14:26:10

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @wordtesteng_OpeningFcn, ...
                   'gui_OutputFcn',  @wordtesteng_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before wordtesteng is made visible.
function wordtesteng_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to wordtesteng (see VARARGIN)

% Choose default command line output for wordtesteng
...
% UIWAIT makes wordtesteng wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = wordtesteng_OutputFcn(hObject, eventdata, handles) 
...
set(handles.text_eng, 'String', handles.mydata.word_list(word_ind_wordtesteng));
...
selected=[handles.mydata.word_list(word_ind_wordtesteng,2) handles.mydata.ans_list(ans_ind(1),2) handles.mydata.ans_list(ans_ind(2),2) handles.mydata.ans_list(ans_ind(3),2)];
btn_ind=randperm(4);
...
for i=1:4
    ...
    if(strcmp(selected{btn_ind(i)},handles.mydata.word_list{word_ind_wordtesteng,2})==1)
...
global word_ind_wordtesteng ans_num_wordtesteng correct_set incorrect_set;
