function varargout = addstudy_basic_plan(varargin)
% ADDSTUDY_BASIC_PLAN MATLAB code for addstudy_basic_plan.fig
%      ADDSTUDY_BASIC_PLAN, by itself, creates a new ADDSTUDY_BASIC_PLAN or raises the existing
%      singleton*.
%
%      H = ADDSTUDY_BASIC_PLAN returns the handle to a new ADDSTUDY_BASIC_PLAN or the handle to
%      the existing singleton*.
%
%      ADDSTUDY_BASIC_PLAN('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ADDSTUDY_BASIC_PLAN.M with the given input arguments.
%
%      ADDSTUDY_BASIC_PLAN('Property','Value',...) creates a new ADDSTUDY_BASIC_PLAN or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before addstudy_basic_plan_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to addstudy_basic_plan_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help addstudy_basic_plan

% Last Modified by GUIDE v2.5 12-Dec-2013 17:19:37

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @addstudy_basic_plan_OpeningFcn, ...
                   'gui_OutputFcn',  @addstudy_basic_plan_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before addstudy_basic_plan is made visible.
function addstudy_basic_plan_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to addstudy_basic_plan (see VARARGIN)

% Choose default command line output for addstudy_basic_plan
...
% UIWAIT makes addstudy_basic_plan wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = addstudy_basic_plan_OutputFcn(hObject, eventdata, handles) 
...
if(word_ind_basic_ap~=1)
    word_ind_basic_ap=word_ind_basic_ap-1;
...
word_more=strcat('http://alldic.nate.com/search/endic_sd.html?q=',handles.mydata.word_list{word_ind_basic_ap,1});
...
while (word_ind_basic_ap<=handles.mydata.word_size)
    
    word_now=word_ind_basic_ap;
    waitfor(handles.tog_more, 'Value', 0);
    
    if word_ind_basic_ap>handles.mydata.word_size
        break;
    end
...
if (word_ind_basic_ap > handles.mydata.word_size)
    ...
    if(a==0)
        ...
        wordtesteng_additional_plan(handles.mydata.user_name, temp.mydata.word_list, handles.mydata.user_additional);
    else
        word_ind_basic_ap=1;
        %guidata(handles.output, handles);
        Control(handles);
...
word_eng=handles.mydata.word_list{word_ind_basic_ap,1};
word_kor=handles.mydata.word_list{word_ind_basic_ap,2};
temp=getappdata(handles.output, 'UsedByGUIData_m');
word_favorite=temp.mydata.word_list{word_ind_basic_ap,11};%handles.mydata.word_list{word_ind_basic_ap,11};
word_now=word_ind_basic_ap;

top_index=strcat(num2str(word_ind_basic_ap), {'  /  '}, num2str(handles.mydata.word_size));
...
if(word_now~=word_ind_basic_ap||get(handles.tog_more, 'Value')==1)
    return;
end
...
%word_favorite=handles.mydata.word_list{word_ind_basic_ap,11};
...
if get(hObject, 'Value') == 1
    handles.mydata.word_list{word_ind_basic_ap,11}=1;
    set(handles.tog_favorite, 'CDATA', handles.mydata.tog_favorite_on);
else
    handles.mydata.word_list{word_ind_basic_ap,11}=0;
    set(handles.tog_favorite, 'CDATA', handles.mydata.tog_favorite_off);
end
...
word_ind_basic_ap=handles.mydata.word_size;
