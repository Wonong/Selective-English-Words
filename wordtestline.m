function varargout = wordtestline(varargin)
% WORDTESTLINE MATLAB code for wordtestline.fig
%      WORDTESTLINE, by itself, creates a new WORDTESTLINE or raises the existing
%      singleton*.
%
%      H = WORDTESTLINE returns the handle to a new WORDTESTLINE or the handle to
%      the existing singleton*.
%
%      WORDTESTLINE('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in WORDTESTLINE.M with the given input arguments.
%
%      WORDTESTLINE('Property','Value',...) creates a new WORDTESTLINE or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before wordtestline_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to wordtestline_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help wordtestline

% Last Modified by GUIDE v2.5 08-Dec-2013 13:28:59

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @wordtestline_OpeningFcn, ...
                   'gui_OutputFcn',  @wordtestline_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before wordtestline is made visible.
function wordtestline_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to wordtestline (see VARARGIN)

% Choose default command line output for wordtestline
...
global isdrawon pagesid totalpages answers iscontrol;

totalpages=ceil(handles.mydata.word_size/DISP_NUM);
pagesid=1;
answers=0;
...
if mod(handles.mydata.word_size, DISP_NUM)==0
    handles.mydata.eng_rand{totalpages}=randperm(10);
    handles.mydata.kor_rand{totalpages}=randperm(10);
else
    handles.mydata.eng_rand{totalpages}=randperm(mod(handles.mydata.word_size, DISP_NUM));
    handles.mydata.kor_rand{totalpages}=randperm(mod(handles.mydata.word_size, DISP_NUM));
end
...
% UIWAIT makes wordtestline wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = wordtestline_OutputFcn(hObject, eventdata, handles) 
...
global isdrawon pagesid totalpages;
...
if pagesid==totalpages
    
    numtodisp=size(handles.mydata.eng_rand{pagesid});
    ...
    for i=1:numtodisp
        set(handles.mydata.eng_set(handles.mydata.eng_rand{pagesid}(i)), 'UserData', DISP_NUM*(pagesid-1)+i);
        set(handles.mydata.kor_set(handles.mydata.kor_rand{pagesid}(i)), 'UserData', DISP_NUM*(pagesid-1)+i);
        
        set(handles.mydata.eng_set(handles.mydata.eng_rand{pagesid}(i)), 'String', handles.mydata.word_list{DISP_NUM*(pagesid-1)+i,1});
        set(handles.mydata.kor_set(handles.mydata.kor_rand{pagesid}(i)), 'String', handles.mydata.word_list{DISP_NUM*(pagesid-1)+i,2});
    end
...
global selected answers pagesid totalpages btn_num2;% iscontrol;
...
if ans_index==selected
    ...
    if strcmp(type, 'eng')==1
        ...
        set(handles.mydata.kor_set(btn_num2), 'UserData', 0);
        set(handles.mydata.kor_set(btn_num2), 'Enable', 'inactive', 'ForegroundColor', [0 0 0], 'CDATA', handles.mydata.choice2);
        line_this{1}=[0;1];
        line_this{2}=[0.05+(10-btn_num)/10;0.05+(10-btn_num2)/10];
        ...
        for i=1:numnow
            ...
            if(get(handles.mydata.kor_set(i), 'UserData')~=0&&i~=btn_num2)
                set(handles.mydata.kor_set(i), 'Enable', 'inactive');
            end
        end

    else
        set(handles.mydata.eng_set(btn_num2), 'UserData', 0);
        set(handles.mydata.eng_set(btn_num2), 'Enable', 'inactive', 'ForegroundColor', [0 0 0], 'CDATA', handles.mydata.choice2);
        ...
        line_this{1}=[0;1];
        line_this{2}=[0.05+(10-btn_num2)/10;0.05+(10-btn_num)/10];
        handles.mydata.line_data{1}=horzcat(handles.mydata.line_data{1}, line_this{1});
        handles.mydata.line_data{2}=horzcat(handles.mydata.line_data{2}, line_this{2});
        
        for i=1:numnow
            if(get(handles.mydata.eng_set(i), 'UserData')~=0&&i~=btn_num2)
                set(handles.mydata.eng_set(i), 'Enable', 'inactive');
            end
...
set(handles.text_number, 'String', strcat(num2str(answers+1), {'  /  '}, num2str(handles.mydata.TESTNUM)));
...
if answers==numnow
    ...
    if pagesid==totalpages
        ...
        wordtestkor(handles.mydata.user_name, handles.mydata.word_list, handles.mydata.point); 
        % go to another test
...
if (get(handles.tog_func, 'Value')==0)
Control(selected, 'eng', 2, handles);
else
    set(handles.tog_func, 'Value', 0);
end
...
if (get(handles.tog_func, 'Value')==0)
Control(selected, 'eng', 3, handles);
else
    set(handles.tog_func, 'Value', 0);
end
...
if (get(handles.tog_func, 'Value')==0)
Control(selected, 'eng', 4, handles);
else
    set(handles.tog_func, 'Value', 0);
end
...
if (get(handles.tog_func, 'Value')==0)
Control(selected, 'eng', 5, handles);
else
    set(handles.tog_func, 'Value', 0);
end
...
if (get(handles.tog_func, 'Value')==0)
Control(selected, 'eng', 6, handles);
else
    set(handles.tog_func, 'Value', 0);
end
...
if (get(handles.tog_func, 'Value')==0)
Control(selected, 'eng', 7, handles);
else
    set(handles.tog_func, 'Value', 0);
end
...
if (get(handles.tog_func, 'Value')==0)
Control(selected, 'eng', 8, handles);
else
    set(handles.tog_func, 'Value', 0);
end
...
if (get(handles.tog_func, 'Value')==0)
Control(selected, 'eng', 9, handles);
else
    set(handles.tog_func, 'Value', 0);
end
...
if (get(handles.tog_func, 'Value')==0)
Control(selected, 'eng', 10, handles);
else
    set(handles.tog_func, 'Value', 0);
end
...
if (get(handles.tog_func, 'Value')==0)
Control(selected, 'kor', 1, handles);
else
    set(handles.tog_func, 'Value', 0);
end
...
if (get(handles.tog_func, 'Value')==0)
Control(selected, 'kor', 2, handles);
else
    set(handles.tog_func, 'Value', 0);
end
...
if (get(handles.tog_func, 'Value')==0)
Control(selected, 'kor', 3, handles);
else
    set(handles.tog_func, 'Value', 0);
end
...
if (get(handles.tog_func, 'Value')==0)
Control(selected, 'kor', 4, handles);
else
    set(handles.tog_func, 'Value', 0);
end
...
if (get(handles.tog_func, 'Value')==0)
Control(selected, 'kor', 5, handles);
else
    set(handles.tog_func, 'Value', 0);
end
...
if (get(handles.tog_func, 'Value')==0)
Control(selected, 'kor', 6, handles);
else
    set(handles.tog_func, 'Value', 0);
end
...
if (get(handles.tog_func, 'Value')==0)
Control(selected, 'kor', 7, handles);
else
    set(handles.tog_func, 'Value', 0);
end
...
if (get(handles.tog_func, 'Value')==0)
Control(selected, 'kor', 8, handles);
else
    set(handles.tog_func, 'Value', 0);
end
...
if (get(handles.tog_func, 'Value')==0)
Control(selected, 'kor', 9, handles);
else
    set(handles.tog_func, 'Value', 0);
end
...
if (get(handles.tog_func, 'Value')==0)
Control(selected, 'kor', 10, handles);
else
    set(handles.tog_func, 'Value', 0);
end
...
% --- Executes on button press in btn_testselected.
function btn_testselected_Callback(hObject, eventdata, handles)
% hObject    handle to btn_testselected (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
