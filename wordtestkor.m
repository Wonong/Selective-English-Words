function varargout = wordtestkor(varargin)
% WORDTESTKOR MATLAB code for wordtestkor.fig
%      WORDTESTKOR, by itself, creates a new WORDTESTKOR or raises the existing
%      singleton*.
%
%      H = WORDTESTKOR returns the handle to a new WORDTESTKOR or the handle to
%      the existing singleton*.
%
%      WORDTESTKOR('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in WORDTESTKOR.M with the given input arguments.
%
%      WORDTESTKOR('Property','Value',...) creates a new WORDTESTKOR or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before wordtestkor_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to wordtestkor_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help wordtestkor

% Last Modified by GUIDE v2.5 08-Dec-2013 18:20:38

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @wordtestkor_OpeningFcn, ...
                   'gui_OutputFcn',  @wordtestkor_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before wordtestkor is made visible.
function wordtestkor_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to wordtestkor (see VARARGIN)

% Choose default command line output for wordtestkor
...
global correct_set incorrect_set combo_set;
...
correct_set=[handles.mydata.correct_img1 handles.mydata.correct_img2];
incorrect_set=[handles.mydata.incorrect_img1 handles.mydata.incorrect_img2];
...
% UIWAIT makes wordtestkor wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = wordtestkor_OutputFcn(hObject, eventdata, handles) 
...
global word_ind_wordtesteng ans_num_wordtesteng;

text_top=strcat(num2str(word_ind_wordtesteng), {'  /  '}, num2str(handles.mydata.word_size));
set(handles.text_eng, 'String', handles.mydata.word_list(word_ind_wordtesteng,2));
...
selected=[handles.mydata.word_list(word_ind_wordtesteng,1) handles.mydata.ans_list(ans_ind(1),1) handles.mydata.ans_list(ans_ind(2),1) handles.mydata.ans_list(ans_ind(3),1)];
btn_ind=randperm(4);
...
for i=1:4
    ...
    if(strcmp(selected{btn_ind(i)},handles.mydata.word_list{word_ind_wordtesteng,1})==1)
        set(btn_num(i),'CallBack',@(x,y)Correct(handles));
        ans_num_wordtesteng=i;
...
% For Test : 
word_ind_wordtesteng=word_ind_wordtesteng+1;
...
global word_ind_wordtesteng ans_num_wordtesteng correct_set incorrect_set combo_set;

btn_num=[handles.btn_num1 handles.btn_num2 handles.btn_num3 handles.btn_num4];

for i=1:4
    if(i~=ans_num_wordtesteng)
...
set(correct_set(1), 'Visible', 'off');
set(correct_set(2), 'Visible', 'off');
set(combo_set(3), 'Visible', 'off');
set(handles.mydata.combo_num, 'Visible', 'off');

if(word_ind_wordtesteng==handles.mydata.word_size+1)
    %go to result
    ...
    wordtestresult2(handles.mydata.user_name, handles.mydata.word_list, handles.mydata.point);
else
    Test(handles);
end
...
for i=1:20
    set(incorrect_set(1), 'Visible', 'on');
    set(incorrect_set(2), 'Visible', 'off');

    pause(0.05);
   
    set(incorrect_set(1), 'Visible', 'off');
    set(incorrect_set(2), 'Visible', 'on');
...
handles.mydata.word_list{word_ind_wordtesteng-1, 9}=handles.mydata.word_list{word_ind_wordtesteng-1, 9}+1;
...
word_ind_wordtesteng=handles.mydata.word_size;
guidata(handles.output, handles);
Test(handles);
